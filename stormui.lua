...
local function normalizeKeyList(keys, caseSensitive)
	...
	if typeof(keys) == "table" then
		for _, entry in ipairs(keys) do
			local text = normalizeKeyText(entry, caseSensitive)
			if text ~= "" then
				table.insert(normalized, text)
			end
		end
	elseif keys ~= nil then
		local text = normalizeKeyText(keys, caseSensitive)
		if text ~= "" then
			table.insert(normalized, text)
		end
	end
...
function StormUI:CreateWindow(options)
	...
	local loadingSubtitle = options.LoadingSubtitle or "initializing interface"
	local loadingDuration = tonumber(options.LoadingDuration) or 1.15
	...
	local keyList = normalizeKeyList(keySystem.Keys or keySystem.Key, keyCaseSensitive)
	local keySuccessText = keySystem.SuccessText or "Klucz poprawny. Trwa otwieranie..."
	local keyInvalidText = keySystem.InvalidText or "Niepoprawny klucz."
	local keyEmptyText = keySystem.EmptyText or "Wpisz klucz, aby kontynuowac."
	local keyErrorText = keySystem.ErrorText or "Wystapil blad podczas sprawdzania klucza."
	...
	local loadingOverlay = create("Frame", {
		...
		BackgroundTransparency = showLoading and 0.18 or 1,
		BorderSizePixel = 0,
	...
	corner(loadingCard, 16)
	stroke(loadingCard, StormUI.Theme.Stroke, 1.2, 0)
	...
	local keyCard = create("Frame", {
		...
		AnchorPoint = Vector2.new(0.5, 0.5),
		Position = UDim2.fromScale(0.5, 0.5),
		Size = UDim2.fromOffset(410, 230),
		ZIndex = 51,
	})
	corner(keyCard, 16)
	stroke(keyCard, StormUI.Theme.Stroke, 1.2, 0)
	...
	local keySubtitleLabel = create("TextLabel", {
		Parent = keyCard,
		BackgroundTransparency = 1,
		AnchorPoint = Vector2.new(0.5, 0),
		...
		Font = Enum.Font.Gotham,
		Text = keySystem.Subtitle or "Wpisz klucz, aby kontynuowac",
		TextSize = 12,
		TextColor3 = StormUI.Theme.Muted,
		TextXAlignment = Enum.TextXAlignment.Center,
		ZIndex = 52,
	...
	local keyNoteLabel = create("TextLabel", {
		...
		Font = Enum.Font.Gotham,
		Text = keySystem.Note or "Dostep do skryptu wymaga poprawnego klucza.",
		TextSize = 11,
		TextColor3 = StormUI.Theme.Muted,
		TextXAlignment = Enum.TextXAlignment.Center,
		ZIndex = 52,
	...
	local keyInput = create("TextBox", {
		...
		Font = Enum.Font.Gotham,
		PlaceholderText = keySystem.Placeholder or "wpisz klucz...",
		Text = "",
		TextSize = 13,
		TextColor3 = StormUI.Theme.Text,
	...
	local keySubmitButton = create("TextButton", {
		Parent = keyCard,
		BackgroundColor3 = StormUI.Theme.Surface,
		BorderSizePixel = 0,
		AnchorPoint = Vector2.new(0.5, 1),
		...
		Text = keySystem.ButtonText or "Sprawdz",
		Font = Enum.Font.GothamSemibold,
		TextSize = 13,
		TextColor3 = StormUI.Theme.Text,
		ZIndex = 52,
	})
	corner(keySubmitButton, 10)
	stroke(keySubmitButton, StormUI.Theme.Stroke, 1, 0)
	bindButtonStates(keySubmitButton, StormUI.Theme.Surface, StormUI.Theme.SurfaceHover, StormUI.Theme.SurfacePressed)
	...
	local pageList = {}
	local tabButtons = {}
	local windowVisible = not keySystemEnabled
	local keyVerified = not keySystemEnabled
	local loadingActive = showLoading
	local keyPromptActive = false
	local keyFlag = keySystem.Flag or "KeySystemVerified"
	setFlag(keyFlag, keyVerified)

	local function updateToggleHint()
		toggleHint.Text = "TOGGLE : " .. toggleKey.Name
	end

	local function validateKeyEntry(rawValue)
		local enteredText = trimText(rawValue)
		if enteredText == "" then
			return false, keyEmptyText, enteredText
		end

		if keySystem.Validator then
			local ok, result, message = pcall(keySystem.Validator, enteredText)
			if not ok then
				return false, keyErrorText, enteredText
			end

			if result == true then
				return true, message or keySuccessText, enteredText
			end

			return false, message or keyInvalidText, enteredText
		end

		local normalizedAttempt = normalizeKeyText(enteredText, keyCaseSensitive)
		for _, allowedKey in ipairs(keyList) do
			if normalizedAttempt == allowedKey then
				return true, keySuccessText, enteredText
			end
		end

		return false, keyInvalidText, enteredText
	end

	local function setKeyStatus(text, success)
		keyStatusLabel.Text = text or ""
		keyStatusLabel.TextColor3 = success == true and StormUI.Theme.Accent or (text ~= "" and Color3.fromRGB(255, 132, 132) or StormUI.Theme.Muted)
	end
	...
	local function finishStartup()
		...
		if keySystemEnabled and not keyVerified then
			showKeyPrompt()
			return
		end
	...
	function window:SetVisible(state)
		...
		if keyVerified and not keyPromptActive and not loadingActive then
			main.Visible = windowVisible
		end
	end

	function window:Toggle()
		if not keyVerified or keyPromptActive or loadingActive then
			return false
		end
	...
	function window:SubmitKey(rawValue)
		if not keySystemEnabled then
			return true, "Key system is disabled."
		end

		local success, message, enteredText = validateKeyEntry(rawValue or keyInput.Text)
		setFlag(keyFlag, success)
		setKeyStatus(message, success)

		if keySystem.Callback then
			keySystem.Callback(success, enteredText)
		end

		if success then
			keyVerified = true
			windowVisible = true
			tween(keyOverlay, 0.16, {BackgroundTransparency = 1})
			tween(keyCard, 0.16, {Size = UDim2.fromOffset(410, 0)})
			task.delay(0.18, function()
				if keyOverlay and keyOverlay.Parent then
					keyOverlay.BackgroundTransparency = 0.18
					keyCard.Size = UDim2.fromOffset(410, 230)
					showMain()
				end
			end)
		end
	...
	if showLoading then
		...
		tween(loadingFill, loadingDuration, {Size = UDim2.new(1, 0, 1, 0)}, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
		task.delay(loadingDuration, function()
			if not loadingOverlay or not loadingOverlay.Parent then
				finishStartup()
				return
			end

			tween(loadingOverlay, 0.18, {BackgroundTransparency = 1})
			tween(loadingCard, 0.18, {Size = UDim2.fromOffset(360, 0)})
			tween(loadingTitleLabel, 0.14, {TextTransparency = 1})
			tween(loadingSubtitleLabel, 0.14, {TextTransparency = 1})
			task.delay(0.2, finishStartup)
		end)
	else
		finishStartup()
	end
